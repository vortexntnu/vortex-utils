...
class PressureToDepthNode : public rclcpp::Node {
   public:
    PressureToDepthNode() : Node("pressure_to_depth") {
        ...
        transform_to_base_link_ =
            this->get_parameter("transform_to_base_link").as_bool();
   ...
   private:
    ...
    bool ensure_tf() {
        ...
        try {
            auto tf = tf_buffer_->lookupTransform(
                "nautilus/base_link", "nautilus/pressure_sensor_link",
                tf2::TimePointZero);
            sensor_to_base_z_ = tf.transform.translation.z;
            RCLCPP_INFO(this->get_logger(),
                        "Pressure sensor z offset from base_link: %.4f m",
                        *sensor_to_base_z_);
            return true;
        } catch (const tf2::TransformException& ex) {
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                                 "Waiting for pressure sensor TF: %s",
                                 ex.what());
            return false;
        }
    }
    ...
    void callback(const sensor_msgs::msg::FluidPressure::SharedPtr msg) {
        if (transform_to_base_link_ && !ensure_tf())
            return;

        double depth =
            (msg->fluid_pressure - atm_pressure_) / (water_density_ * gravity_);

        // Shift depth from sensor frame to base_link: sensor is at z offset
        // from base_link, so base_link depth = sensor depth - z_offset
        // (negative z = sensor is above base_link).
        if (sensor_to_base_z_.has_value()) {
